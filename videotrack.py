...
class Model:
    ...
    def __init__(self):
        ...
        self.upper_bounds = [self.purple_upper, self.lpink_upper, self.dgreen_upper,
                            self.yellow_upper, self.red_upper,  self.blue_upper,
                            self.lgreen_upper]
        self.lower_bounds = [self.purple_lower, self.lpink_lower, self.dgreen_lower,
                            self.yellow_lower, self.red_lower,  self.blue_lower,
                            self.lgreen_lower]
        ...
        self.plot_colors = ['k', 'm', 'g', 'y', 'r', 'b', 'c']
    ...
    def process_input(self, input_buffer):
        ...
        pixel = hsv_frame[int(command[2]), int(command[1])] # grab the pixel from the center of the calibration circle
        print(pixel)
    ...
    def show_data(self, x, y, color):
        ...
        if color == 0:
            ...
            self.ax.set_ylim(0, 540)

        self.ax.plot(self.xdata[color], self.frame.shape[0]-self.ydata[color], color=self.plot_colors[color])
        plt.draw()
...
def main_loop(calibrate=False, fname='data.npy', scale=True):
    """
        ...
        if calibrate:
            ...
            if not input_queue.empty():
                ...
                if len(input_buffer) > 0:
                    ...
                    command = model.process_input(input_buffer) #LOC input
                    ...
                    print("Enter <color, x_coord, y_coord> to calibrate new color.")

        _, model.frame = cap.read() # get a frame from the camera, if we aren't calibrating
        if model.frame is not None: #Should save when video ends now
            if scale:
                model.frame = cv2.resize(model.frame, (int(model.frame.shape[1]/1.5), int(model.frame.shape[0]/1.5)))
...
if __name__ == '__main__':
    ...
    if len(sys.argv) >1:
        if str(sys.argv[1]) == '-c':
            ...
            main_loop(calibrate=True,fname='data.npy', scale=True)
        else:
            print("Invalid argument")
    else:
        main_loop(fname='data.npy', scale=False)
